...
module.exports.setupAccessControl = function(store) {
  ...
  store.readPathAccess('swims.*', function() {
  ...
  store.writeAccess('*', 'swims.*', function() {
    if (!(this.session && this.session.userId)) {
      return;
    }
    var next = arguments[arguments.length - 1];
    return next(this.session.userId === process.env.ADMIN_ID)
  });

  store.readPathAccess('config.swims.*', function() {
    var next = arguments[arguments.length - 1];
    return next(true);
  });
  store.writeAccess('*', 'config.swims.*', function() {
    if (!(this.session && this.session.userId)) {
      return;
    }
    var next = arguments[arguments.length - 1];
