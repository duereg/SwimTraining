...
get('/home', function(page, model, params) { 
  model.subscribe("swims.*", function(err, swims) {
    logger.log("swims changed");
...
get('/calendar', function(page, model, params) { 
  model.subscribe("swims.*", function(err, swims) { 
    model.refList("_swims", "swims", "config.swims.dates");

    logger.log(model.get("_swims"));
...
ready(function(model) { 
  ...
  exports.addSwim = function() {
    ...
    model.push('config.swims.dates', date);
    model.set('swims.' + date + '.text', text);
    model.set('swims.' + date + '.workout', translated);
    model.set('swims.' + date + '.date', date); 
    model.set('swims.' + date + '.distance', translated.totalDistance());
    model.del("swims.new");
